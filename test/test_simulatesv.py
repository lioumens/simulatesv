...
class TestChanges(unittest.TestCase):
    ...
    def test_snps_ref_idx_dna(self):
        ...
        for i in range(len(self.changes)):
            try:
                # subTest is a new feature in Python 3.4, useful for identifying
        ...
        def check_changes():
            ...
            for j, ins_entry in enumerate(self.changes[i]):
                if ins_entry[0] == "INS":
                    ...
                    ref_dna = self.read_bases("reference.fna", ref_idx)
                    alt_dna = self.read_bases("sim_{0}.fna".format(i), alt_idx)
                    error_message = "ref INS on line {0} is not correct".format(j + 2)
        ...
        def check_changes():
            ...
            for j, cnv_entry in enumerate(self.changes[i]):
                if cnv_entry[0] == "CNV":
                    ...
                    sim_genome = "sim_{0}.fna".format(i)
                    alt_dna = self.read_bases(sim_genome, alt_idx_start, alt_idx_end)
                    error_message = "alt CNV on line {0} does not match".format(j + 2)
                    self.assertEqual(alt_dna, change_dna, error_message)
        for i in range(len(self.changes)):
            try:
                with self.subTest(i=i):
        ...
        def check_changes():
            ...
            for j, trans_entry in enumerate(self.changes[i]):
                if trans_entry[0] == "TRANS":
                    ...
                    sim_genome = "sim_{0}.fna".format(i)
                    alt_dna = self.read_bases(sim_genome, alt_idx_start, alt_idx_end)
                    error_message = "alt TRANS on line {0} does not match".format(j + 2)
                    self.assertEqual(alt_dna, change_dna, error_message)
        for i in range(len(self.changes)):
            try:
                with self.subTest(i=i):
        ...
        def check_changes():
            ...
            for j, trans_entry in enumerate(self.changes[i]):
                if trans_entry[0] == "TRANS":
                    ...
                    ref_dna = self.read_bases("reference.fna", ref_idx_start, ref_idx_end)
                    error_message = "ref TRANS on line {0} does not match".format(j + 2)
                    self.assertEqual(ref_dna, change_dna, error_message)
        for i in range(len(self.changes)):
            try:
                with self.subTest(i=i):
